...
- (void)labelCountUpAnimationWithRate:(CGFloat)rate
{
    POPAnimatableProperty *prop = [POPAnimatableProperty propertyWithName:@"countup" initializer:^(POPMutableAnimatableProperty *prop) {
        ...
        prop.writeBlock = ^(id obj, const CGFloat values[]){
            ...
            [self setLabelTimeWithTimeLeft:timeSeconds];
        };
    }];
    POPBasicAnimation *anBasic = [POPBasicAnimation linearAnimation];//秒表当然必须是线性的时间函数
    anBasic.property = prop;//自定义属性
...
- (void)animationWithStrokeEnd:(CGFloat)strokeEnd labelAutoCountDownAnimation:(BOOL)autoCountDown;
{
    ...
    [strokeEndAnim setCompletionBlock:^(POPAnimation *anim, BOOL finished) {
        if (finished) {
            if (self.cirCleType == CirCleTypeLabel && autoCountDown) {
                [self labelCountDownAnimationWithRate:strokeEnd];
            }else{
    ...
    [self setLabelTimeWithTimeLeft:strokeEnd * self.maxSecond];
